...
for i in range(1,int(num_images/2)):
    t = str(i)
    ChessImaR = cv2.imread('images/chessboard-R' + t + '.png', 0)    # Right side
    ChessImaL = cv2.imread('images/chessboard-L' + t + '.png', 0)    # Left side
    retR, cornersR = cv2.findChessboardCorners(ChessImaR, chessSize, None)  # Define the number of chees corners we are looking for
    retL, cornersL = cv2.findChessboardCorners(ChessImaL, chessSize, None)  # Left side
...
cv2.resizeWindow('Filtered Color Depth',1000,1000)
